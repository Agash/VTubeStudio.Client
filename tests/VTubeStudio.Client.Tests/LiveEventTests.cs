...
public sealed class LiveEventTests : LiveTestBase
{
    ...
    public async Task ApiErrors_MapToTypedIds()
    {
        ...
        VTubeStudioClient client = Client;
        {
            await ExpectApiErrorAsync(
                151,
                () => client.LoadModelAsync(new ModelLoadRequest { ModelId = "xyz" }, cts.Token)
            );
            CurrentModelResponse current = await client.GetCurrentModelAsync(cts.Token);
            await ExpectApiErrorAsync(
                current.ModelLoaded ? 202 : 251,
                () =>
                    client.TriggerHotkeyAsync(
                        new HotkeyTriggerRequest { HotkeyId = "no-such-hotkey" },
                        cts.Token
                    )
            );
            await ExpectApiErrorAsync(
                401,
                () =>
                    client.DeleteParameterAsync(
                        new ParameterDeletionRequest { ParameterName = "Nope1234" },
                        cts.Token
                    )
            );
            await ExpectApiErrorAsync(
                950,
                () => client.SubscribeAsync("NoSuchEvent", ct: cts.Token)
            );
        }
    }
    ...
    public async Task ApiState_ReportsVersion()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            TestContext.WriteLine(
                $"VTube Studio version: {state.VTubeStudioVersion}, active={state.Active}, authenticated={state.CurrentSessionAuthenticated}"
            );
            ...
            Assert.IsTrue(
                state.CurrentSessionAuthenticated,
                "Session should be authenticated after AuthenticateAsync."
            );
        }
    }
    ...
    public async Task TestEvent_RawSubscriber_ReceivesTicks()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            {
                TestContext.WriteLine(
                    $"tick: data={e.RawData.GetRawText()} at={e.ReceivedAtUtc:O}"
                );
                Assert.AreEqual("cfg", e.RawData.GetProperty("yourTestMessage").GetString());
            }

            EventSubscriptionResponse unsub = await client.SubscribeAsync(
                VTubeStudioEventNames.Test,
                subscribe: false,
                cts.Token
            );
    ...
    public async Task ModelLoadedEvent_TypedAndRaw_FireOnProgrammaticSwap()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            TestContext.WriteLine(
                $"Current: loaded={before.ModelLoaded} name={before.ModelName} id={before.ModelId}; available={available.AvailableModels.Count}"
            );
            AvailableModel? other = available.AvailableModels.FirstOrDefault(m =>
                m.ModelId != before.ModelId
            );
            Assert.IsNotNull(
                other,
                "Need at least 2 models in VTube Studio to exercise a programmatic model swap."
            );
            ...
            try
            {
                EventSubscriptionResponse sub =
                    await client.SubscribeAsync<ModelLoadedEventPayload>(ct: cts.Token);
                TestContext.WriteLine($"Subscribed: [{string.Join(",", sub.SubscribedEvents)}]");
                Assert.IsTrue(
                    sub.SubscribedEvents.Contains(VTubeStudioEventNames.ModelLoaded),
                    Diagnostic("subscribe-ack", sub, typed, raw)
                );

                ModelLoadResponse load = await client.LoadModelAsync(
                    new ModelLoadRequest { ModelId = other!.ModelId },
                    cts.Token
                );
                ...
                Assert.IsTrue(
                    payloads.Any(p => !p.ModelLoaded),
                    $"Expected an unload event. Got: {Describe(payloads)}"
                );
                Assert.IsTrue(
                    payloads.Any(p => p.ModelLoaded && p.ModelId == other.ModelId),
                ...
                if (before.ModelLoaded && before.ModelId is not null)
                {
                    ...
                    try
                    {
                        await client.LoadModelAsync(
                            new ModelLoadRequest { ModelId = before.ModelId },
                            cts.Token
                        );
                    ...
                    {
                        TestContext.WriteLine(
                            $"WARNING: failed to restore model {before.ModelName}: {ex.Message}"
                        );
                    }
                }
            }
        ...
        static string Describe(IEnumerable<ModelLoadedEventPayload> payloads)
        {
            return string.Join(
                " | ",
                payloads.Select(p => $"loaded={p.ModelLoaded} name={p.ModelName} id={p.ModelId}")
            );
        }
    ...
    public async Task ReversibleWrites_InjectMoveItemRoundtrips()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ParameterInfo faceAngle = await client.GetParameterValueAsync(
                new ParameterValueRequest { Name = "FaceAngleX" },
                cts.Token
            );
            TestContext.WriteLine(
                $"FaceAngleX value={faceAngle.Value} range=[{faceAngle.Min},{faceAngle.Max}]"
            );
            ...
            if (current.ModelLoaded)
            {
                await client.MoveModelAsync(
                    new MoveModelRequest { TimeInSeconds = 0, ValuesAreRelativeToModel = true },
                    cts.Token
                );
            }
            else
            ...
            {
                ItemUnloadResponse unloaded = await UnloadWithRetryAsync(
                    client,
                    loaded.InstanceId,
                    cts.Token
                );
    ...
    private static async Task<ItemUnloadResponse> UnloadWithRetryAsync(
    ...
    )
    {
        for (int attempt = 1; ; attempt++)
        {
            try
            {
                return await client.UnloadItemAsync(
                    new ItemUnloadRequest { InstanceIds = [instanceId] },
                    ct
                );
            }
    ...
    private static async Task<ModelLoadResponse> LoadModelWithCooldownRetryAsync(
        VTubeStudioClient client,
        string modelId,
        CancellationToken ct
    )
    {
        for (int attempt = 0; ; attempt++)
        {
            try
            {
                ...
                when (ex.ErrorId == VTubeStudioErrorId.ModelLoadCooldownNotOver && attempt < 4)
            {
    ...
    public async Task StableReads_FoldersScenePhysicsPostProcessingPermissions()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            SceneColorOverlayInfoResponse overlay = await client.GetSceneColorOverlayInfoAsync(
                cts.Token
            );
            TestContext.WriteLine($"overlay: active={overlay.Active}");

            GetCurrentModelPhysicsResponse physics = await client.GetCurrentModelPhysicsAsync(
                cts.Token
            );
            TestContext.WriteLine(
                $"physics: loaded={physics.ModelLoaded} hasPhysics={physics.ModelHasPhysics} groups={physics.PhysicsGroups.Count}"
            );
            ...
            TestContext.WriteLine(
                $"post: supported={post.PostProcessingSupported} active={post.PostProcessingActive}"
            );

            PostProcessingUpdateResponse updated = await client.UpdatePostProcessingAsync(
                new PostProcessingUpdateRequest
                {
            ...
            PermissionResponse permissions = await client.RequestPermissionAsync(
                timeout: TimeSpan.FromSeconds(30),
                ct: cts.Token
            );
            TestContext.WriteLine(
                $"permissions: [{string.Join(",", permissions.Permissions.Select(p => $"{p.Name}={p.Granted}"))}]"
            );
    ...
    public async Task ParameterCreateInjectDelete_Roundtrip()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            try
            {
                ...
                Assert.IsTrue(
                    list.CustomParameters.Any(p => p.Name == name),
                    "Created parameter missing from input list."
                );
            ...
            finally
            {
                ...
                {
                    TestContext.WriteLine(
                        $"WARNING: failed to delete parameter {name}: {ex.Message}"
                    );
                }
            }
            ...
            Assert.IsFalse(
                after.CustomParameters.Any(p => p.Name == name),
    ...
    public async Task ItemControl_MoveAnimatePinRoundtrip()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            AvailableItemFile? file = files.AvailableItemFiles.FirstOrDefault(f =>
                f.Type != "Live2D"
            );
            if (file is null)
            {
                Assert.Inconclusive(
                    "No non-Live2D item files available; skipping item control roundtrip."
                );
            }
            ...
            try
            {
                ...
                Assert.IsTrue(
                    moved.MovedItems[0].Success,
                    $"Move failed with error {moved.MovedItems[0].ErrorId}."
                );

                ItemAnimationControlResponse anim = await client.ControlItemAnimationAsync(
                    new ItemAnimationControlRequest
                    {
                        ItemInstanceId = loaded.InstanceId,
                        Brightness = 1,
                        Opacity = 1,
                ...
                ItemSortResponse sorted = await client.SortItemAsync(
                    new ItemSortRequest { ItemInstanceId = loaded.InstanceId },
                    cts.Token
                );
        ...
        VTubeStudioClient client = Client;
        {
            ...
            await AttemptOrSkipAsync(
                ...
                async () =>
                {
                    ArtMeshAtPositionResponse at = await client.GetArtMeshesAtPositionAsync(
                        new ArtMeshAtPositionRequest { X = 0, Y = 0 },
                        cts.Token
                    );
            ...
            await AttemptOrSkipAsync(
                ...
                async () =>
                {
                    ...
                    TestContext.WriteLine(
                        $"subscribed: [{string.Join(",", sub.SubscribedEvents)}]"
                    );
            ...
            await AttemptOrSkipAsync(
                ...
                async () =>
                {
                    ...
                    NdiConfigResponse applied = await client.SetNdiConfigAsync(
            ...
            {
                Assert.Inconclusive(
                    $"Unavailable on this VTube Studio build: {string.Join(", ", unavailable)}."
                );
            }
        }
    }
    ...
    public async Task ModelWrites_MovePhysicsExpressionTintRoundtrip()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            if (!before.ModelLoaded)
            {
                ...
                _ = await LoadModelWithCooldownRetryAsync(
                    client,
                    available.AvailableModels[0].ModelId,
                    cts.Token
                );
            ...
            try
            {
                ...
                if (physics is { ModelHasPhysics: true } && physics.PhysicsGroups.Count > 0)
                {
                    ...
                    await client.SetCurrentModelPhysicsAsync(
                        new SetCurrentModelPhysicsRequest
                        {
                            StrengthOverrides =
                            [
                                new PhysicsOverride
                                {
                                    Id = group.GroupId,
                                    Value = group.StrengthMultiplier,
                                    OverrideSeconds = 0.5,
                                },
                            ],
                        },
                ...
                ExpressionStateResponse expressions = await client.GetExpressionStateAsync(
                    ct: cts.Token
                );
                if (expressions.Expressions.Count > 0)
                {
                    ...
                    await client.SetExpressionAsync(
                        new ExpressionActivationRequest { ExpressionFile = file, Active = true },
                        cts.Token
                    );
                    await client.SetExpressionAsync(
                        new ExpressionActivationRequest { ExpressionFile = file, Active = false },
                        cts.Token
                    );
                }
                else
                ...
                if (meshes.NumberOfArtMeshNames > 0)
                {
                    ...
                    _ = await client.TintArtMeshAsync(
                    ...
                    ColorTintResponse reset = await client.TintArtMeshAsync(
                        new ColorTintRequest
                        {
                            ColorTint = new ColorTint
                            {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            try
            {
                foreach (AvailableModel model in available.AvailableModels)
                {
                    ...
                    GetCurrentModelPhysicsResponse physics =
                        await client.GetCurrentModelPhysicsAsync(cts.Token);
                    TestContext.WriteLine(
                        $"{current.ModelName}: params={current.NumberOfLive2DParameters} meshes={meshes.NumberOfArtMeshNames} tags={meshes.NumberOfArtMeshTags} groups={meshes.NumberOfArtMeshGroups} expressions={expressions.Expressions.Count} hasPhysics={physics.ModelHasPhysics} physicsGroups={physics.PhysicsGroups.Count}"
                    );
                    ...
                    {
                        ArtMeshMatcher matcher = new() { TintAll = true };
                        ColorTintResponse tinted = await client.TintArtMeshAsync(
                            new ColorTintRequest
                            {
                                ColorTint = new ColorTint
                                {
                        ...
                        Assert.IsTrue(
                            tinted.MatchedArtMeshes > 0,
                            $"Tint matched nothing on {current.ModelName}."
                        );
                        ColorTintResponse reset = await client.TintArtMeshAsync(
                            new ColorTintRequest
                            {
                        ...
                        Assert.IsTrue(
                            reset.MatchedArtMeshes > 0,
                            $"White reset matched nothing on {current.ModelName}."
                        );
                    }
                }
            }
            ...
            {
                string restoreId = before is { ModelLoaded: true, ModelId: not null }
                    ? before.ModelId
                    : string.Empty;
    ...
    private static async Task AttemptOrSkipAsync(
        string name,
        List<string> unavailable,
        TestContext context,
        Func<Task> attempt
    ...
    public async Task ModelLoadCooldown_MapsToTypedError()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            for (int attempt = 0; ; attempt++)
            {
                try
                {
                    _ = await client.LoadModelAsync(
                        new ModelLoadRequest { ModelId = current.ModelId },
                        cts.Token
                    );
                    break;
                }
                catch (VTubeStudioApiException ex)
                    when (ex.ErrorId == VTubeStudioErrorId.ModelLoadCooldownNotOver && attempt < 3)
                {
            ...
            try
            {
                ...
                Assert.Inconclusive(
                    "Server did not enforce the load cooldown; mapping not verified."
                );
            }
            catch (VTubeStudioApiException ex)
            {
    ...
    public async Task RequestResponse_SanityChecks_StillWorkAlongsideEvents()
    {
        ...
        VTubeStudioClient client = Client;
        {
            ...
            AvailableModelsResponse available = await client.GetAvailableModelsAsync(cts.Token);
            TestContext.WriteLine(
                $"stats: {stats.Framerate}fps uptime={stats.Uptime}; current={current.ModelName}; models={available.AvailableModels.Count}"
            );
            ...
            HotkeysInCurrentModelResponse hotkeysExplicit = await client.GetHotkeysAsync(
                new HotkeysInCurrentModelRequest(),
                cts.Token
            );
            ExpressionStateResponse expressionsDetailed = await client.GetExpressionStateAsync(
                new ExpressionStateRequest { Details = true },
                cts.Token
            );
            ...
            ArtMeshListResponse meshes = await client.GetArtMeshListAsync(cts.Token);
            InputParameterListResponse inputParams = await client.GetInputParametersAsync(
                cts.Token
            );
            Live2DParameterListResponse live2DParams = await client.GetLive2DParametersAsync(
                cts.Token
            );
            ...
            TestContext.WriteLine(
                $"hotkeys={hotkeys.AvailableHotkeys.Count} expressions={expressions.Expressions.Count} meshes={meshes.NumberOfArtMeshNames} input={inputParams.DefaultParameters.Count} live2d={live2DParams.Parameters.Count} items={items.ItemInstancesInScene.Count} face={face.Found}"
            );
            ...
            Assert.IsTrue(
                rawEvents >= 1,
                "Expected background TestEvent ticks while doing request/response calls."
            );
