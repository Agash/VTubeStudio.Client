...
public sealed record TestEventConfig
{
    ...
    [JsonPropertyName("testMessageForEvent")]
    public string? TestMessageForEvent { get; init; }
}
...
public sealed record ModelLoadedEventConfig
{
    ...
    public IReadOnlyList<string>? ModelId { get; init; }
}
...
public sealed record HotkeyTriggeredEventConfig
{
    ...
    [JsonPropertyName("onlyForAction")]
    public string? OnlyForAction { get; init; }

    /// <summary>When true, hotkeys triggered via the API are not reported by the event.</summary>
    [JsonPropertyName("ignoreHotkeysTriggeredByAPI")]
    public bool IgnoreHotkeysTriggeredByApi { get; init; }
}
...
public sealed record ModelAnimationEventConfig
{
    ...
    [JsonPropertyName("ignoreLive2DItems")]
    public bool IgnoreLive2DItems { get; init; }

    /// <summary>When true, idle-animation events are not reported.</summary>
    [JsonPropertyName("ignoreIdleAnimations")]
    public bool IgnoreIdleAnimations { get; init; }
}
...
public sealed record ItemEventConfig
{
    ...
    [JsonPropertyName("itemInstanceIDs")]
    public IReadOnlyList<string>? ItemInstanceIds { get; init; }

    /// <summary>When set, only events for items with the listed file names are delivered.</summary>
    [JsonPropertyName("itemFileNames")]
    public IReadOnlyList<string>? ItemFileNames { get; init; }
}
...
public sealed record ModelClickedEventConfig
{
    ...
    [JsonPropertyName("onlyClicksOnModel")]
    public bool OnlyClicksOnModel { get; init; }
}
...
public sealed record ExpressionToggledEventConfig
{
    ...
    [JsonPropertyName("sendAllActiveStatesOnSubscription")]
    public bool SendAllActiveStatesOnSubscription { get; init; }
...
public sealed record ArtMeshTrackingEventConfig
{
    ...
    public IReadOnlyList<ArtMeshTrackingPoint> TrackingPoints { get; init; } = [];
}
...
public sealed record ArtMeshTrackingPoint
{
    ...
    public required string TrackingPointId { get; init; }

    /// <summary>Barycentric coordinates of the point.</summary>
    [JsonPropertyName("artMeshCoords")]
    public ArtMeshTrackingCoords? ArtMeshCoords { get; init; }

    /// <summary>When true, a visualizer circle is shown at the point.</summary>
    [JsonPropertyName("visualize")]
    public bool Visualize { get; init; }
}
...
public sealed record ArtMeshOutlineEventConfig
{
    ...
    [JsonPropertyName("artMeshes")]
    public IReadOnlyList<ArtMeshOutlineEntry> ArtMeshes { get; init; } = [];
}
