...
public sealed record InputParameterListResponse
{
    ...
    public IReadOnlyList<ParameterInfo> CustomParameters { get; init; } = [];

    /// <summary>Built-in default tracking parameters provided by VTube Studio.</summary>
    [JsonPropertyName("defaultParameters")]
    public IReadOnlyList<ParameterInfo> DefaultParameters { get; init; } = [];
}
...
public sealed record InjectParameterDataRequest
{
    ...
    public string Mode { get; init; } = "set"; // "set" or "add"

    /// <summary>The parameter values to inject.</summary>
    [JsonPropertyName("parameterValues")]
    public required IReadOnlyList<ParameterValue> ParameterValues { get; init; }
}
