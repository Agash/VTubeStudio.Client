...
public sealed record HotkeysInCurrentModelRequest
{
    ...
    [JsonPropertyName("live2DItemFileName")]
    public string? Live2DItemFileName { get; init; }
}
...
public sealed record HotkeysInCurrentModelResponse
{
    ...
    public IReadOnlyList<AvailableHotkey> AvailableHotkeys { get; init; } = [];
}
...
public sealed record AvailableHotkey
{
    ...
    public IReadOnlyList<string> KeyCombination { get; init; } = [];

    /// <summary>The on-screen button number (1-8) the hotkey is mapped to, or -1 when it is not mapped to a button.</summary>
    [JsonPropertyName("onScreenButtonID")]
    public int OnScreenButtonId { get; init; }
}
